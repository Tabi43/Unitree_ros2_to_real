...
GaitGenerator::GaitGenerator(CtrlComponents *ctrlComp)
              : _waveG(ctrlComp->waveGen.get()), _est(ctrlComp->estimator.get()), 
                _phase(ctrlComp->phase.get()), _contact(ctrlComp->contact.get()), 
                _robModel(ctrlComp->robotModel.get()), _state(ctrlComp->lowState.get()){
