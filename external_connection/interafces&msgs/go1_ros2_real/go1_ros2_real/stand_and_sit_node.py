...
Sequence:
  ...
  2. STAND_UP   – linearly interpolate joints to standing position over
                  `transition_steps` control steps  (mirrors State_FixedStand)
  3. HOLD       – hold standing position for `hold_seconds` seconds
  4. SIT_DOWN   – linearly interpolate joints to kennel position over
                  `transition_steps` steps  (mirrors State_FixedKennel)
  5. PASSIVE    – damp-only command (Kp=0, Kd=3) then stop

Gains follow setRealStanceGain / State_Passive from sport_controller.
"""
...
class StandAndSitNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("wait_for_low_state", False)

        self._hold_seconds = self._param("hold_seconds").double_value
        self._transition_steps = self._param(
            "transition_steps"
        ).integer_value
        self._publish_rate = self._param("publish_rate").double_value
        ...
        self._timer = self.create_timer(
            1.0 / self._publish_rate, self._control_loop
        )
    ...
    def _make_low_cmd(self) -> LowCmd:
        ...
        cmd.sn = [0, 0]
        cmd.version = [1, 0]
        cmd.band_width = int(self._publish_rate)
    ...
    def _control_loop(self) -> None:
        ...
        if self._phase == Phase.STAND_UP:
            ...
            self._hold_elapsed += 1.0 / self._publish_rate
            if self._hold_elapsed >= self._hold_seconds:
                self.get_logger().info("Hold complete. Sitting down …")
                # Snapshot current position (should be ~STAND_POS)
                self._start_pos = list(STAND_POS)
                self._step = 0
            ...
            self._pub.publish(self._low_cmd)
            # After one full second of passive commands, stop.
            self._step += 1
            if self._step >= int(self._publish_rate):
